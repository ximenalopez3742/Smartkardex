...
def analizar_alumno(codigo):
    """
    Analiza el kárdex de un alumno y sugiere materias disponibles.

    Acepta un horario activo (materias ya inscritas este ciclo) para:
      1. NO sugerirlas como disponibles (ya están inscritas).
      2. NO sumar sus créditos al avance real por área.
      3. Comparar por CLAVE y por NOMBRE (normalizado).

    El horario puede enviarse de dos formas:
      - POST con body JSON: { "horario": [{"clave": "I5886", "nombre": "CALCULO DIFERENCIAL"}, ...] }
      - GET con query param: ?horario=I5886,I5887  (solo claves, separadas por coma)
    """
    try:
        horario: list[dict] = []
        ...
        if request.method == "POST":
            ...
            # Normalizar: si llegó lista de strings, convertir a dicts con clave
            horario = [
                h if isinstance(h, dict) else {"clave": str(h)}
                for h in horario
            ]

        else:  # GET — claves como query param (retrocompatible)
