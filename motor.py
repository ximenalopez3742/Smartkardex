"""
motor.py — Motor de Inferencia para sugerencias académicas
==========================================================
...
Notas de diseño:
  - Estatus FINAL por materia: si tiene al menos una APROBADA cuenta como
    aprobada, aunque tenga reprobaciones previas → desbloquea prerrequisitos.
  - Art. 33 cuenta periodos DISTINTOS de reprobación, no filas individuales.
  - El horario actual se lee de la tabla `horario` (BD), no del PDF.
"""
...
def _calendarios_consecutivos(calendarios: set) -> bool:
    """Detecta si hay 2 calendarios consecutivos en un set como '2023-A','2023-B'."""
...
class MotorInferencia:
    ...
    def analizar(
        ...
        servicio_social: bool = False,
        practicas: bool = False,
        horario_claves: list = None,   # ignorado — se usa la tabla horario
    ) -> Optional[dict]:
        """
        Retorna el análisis completo del alumno o None si no existe.
        Si el plan de estudios no está cargado retorna un dict con clave "sin_plan".
        """
        ...
        # ── 3. Estatus final + créditos propios ───────────────────
        ...
        # ── 5. Discrepancia con créditos del kardex ───────────────
        ...
        for r in cursor.fetchall():
            ...
            adq_propio = creditos_propios_norm.get(normalizar(r["area"]), 0)
        ...
        plan_creditos_map = {
            str(r["clave"]).strip().upper(): (r["area"], r["creditos"])
            for r in cursor.fetchall()
        }
        ...
        if not plan_rows:
            # Plan no cargado — devolver resultado parcial con aviso
            return {
                "alumno":      alumno,
                "sin_plan":    True,
                "disponibles": [],
                ...
                "alertas":     [{
                    "tipo":        "SIN_PLAN",
                    "icono":       "⚠️",
                    "descripcion": "El plan de estudios no está cargado. "
        ...
        # ── 9. Clasificar disponibles / bloqueadas ────────────────
        disponibles = []
        bloqueadas  = []
        ...
        for row in plan_rows:
            ...
            if orientacion_alumno and ori_cod and ori_cod not in ("AG", "OH"):
                if normalizar(orientacion_alumno) not in (
                    normalizar(ori_cod), normalizar(ori)
                ):
                    continue
        ...
        for a in por_area_raw:
            ...
            for key, val in CREDITOS_REQUERIDOS_AREA.items():
                if normalizar(key) == normalizar(a["area"]):
                    req_oficial = val
                    break
        ...
        alertas    = []
        pct_avance = (
            creditos_propios_total / creditos_requeridos * 100
        ) if creditos_requeridos > 0 else 0

        # Discrepancia total de créditos
        ...
        for disc in discrepancias_area:
            alertas.append({
                ...
                "descripcion": (
                    f"Área '{disc['area']}': kárdex dice {disc['kardex']} cr, "
            ...
            if len(cals) >= MAX_REPROBACIONES:
                ...
                if _calendarios_consecutivos(cals):
                    alertas.append({
                        ...
                        "descripcion": (
                            f"'{info['nombre']}' ({clave}) reprobada en 2 semestres "
                            f"CONSECUTIVOS ({periodos}). Debes solicitar artículo y visitar "
                            "a tu coordinador."
                        ),
                    })
                else:
        ...
        if alumno.get("practicas_profesionales"):
            ...
            alertas.append({
                "tipo": "PRACTICAS_PENDIENTE", "icono": "⏳",
                "descripcion": (
        ...
        return {
            ...
            "discrepancia_creditos": diferencia_creditos,
            "rep_activas": {
                k: {**v, "calendarios": list(v["calendarios"])}
    ...
    def sugerir_materia(self, codigo_alumno: str, query: str, area_manual: str = None) -> dict:
        ...
        1. Por clave exacta
        2. Por nombre similar (difflib, umbral 0.55)
        3. Lista materias de un área si se especifica
        """
        cursor = self.conn.cursor()
        ...
        cursor.execute("""
            SELECT clave, materia, area, area_cod, orientacion, creditos
            FROM plan_estudios
        """)
        ...
        if candidatos and candidatos[0][1] >= 0.55:
            return {
                "encontrado": True,
                "filtro":     "nombre",
                "materia":    candidatos[0][0],
                "similitud":  round(candidatos[0][1], 2),
            }

        # Filtro 3: lista por área manual
        if area_manual:
    ...
    def guardar_horario(self, codigo_alumno: str, materias_horario: list, calendario: str) -> dict:
        """
        Guarda (reemplaza) el horario del alumno para el calendario dado.
