...
- analizar() acepta `materias_en_curso` (lista de dicts con 'clave' y 'nombre')
  para excluirlas de las sugerencias (comparación por clave primero, luego nombre).
- Cuando una materia del kárdex no existe en el plan, se registra en `sin_equivalencia`
  para que la API/frontend pueda pedir los datos al usuario.
- Nuevo método registrar_equivalencia() — inserta la materia en plan_estudios.
- Anotaciones de tipo compatibles con Python 3.8+.
"""
...
def _estatus_final(intentos: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
...
class MotorInferencia:
    ...
    # ─────────────────────────────────────────────────────────────
    # Registrar equivalencia cuando la materia no existe en el plan
    # ─────────────────────────────────────────────────────────────
            ...
            cursor.execute("""
                INSERT OR IGNORE INTO plan_estudios
                    (clave, materia, area_cod, area,
                     orientacion_cod, orientacion, creditos, prerrequisito)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
    ...
    def analizar(
        self,
        codigo_alumno: str,
        materias_en_curso: Optional[List[Dict[str, Any]]] = None,
        ...
        materias_en_curso : list[dict] | None
            Materias que el alumno cursa AHORA según su horario.
            Cada dict debe tener 'clave' (str) y opcionalmente 'nombre' (str).
            No aparecerán en las sugerencias.

        Returns
        -------
        dict con: alumno, disponibles, bloqueadas, alertas, por_area, sin_equivalencia.
        """
        cursor = self.conn.cursor()

        # ── Normalizar materias_en_curso ──────────────────────────
        en_curso_claves: set = set()
        en_curso_nombres: set = set()
        ...
        pct_avance = (creditos_actuales / creditos_requeridos * 100) if creditos_requeridos > 0 else 0.0
        ...
        for r in plan_rows:
            rd = dict(r)
            plan_por_clave[rd["clave"].strip().upper()] = rd
            plan_por_nombre[normalizar(rd["materia"])] = rd

        # ── 5. Materias del alumno sin equivalencia en el plan ────
        sin_equivalencia = []
        for clave, intentos in por_clave.items():
            resultado = _estatus_final(intentos)
        ...
        for row in plan_rows:
            ...
            base = {"clave": clave, "nombre": materia, "area_cod": area_cod,
                    "area": area, "orientacion": ori, "creditos": creditos}

            if not pre:
                disponibles.append({**base, "estado": "Sin prerrequisito"})
            else:
                pre_norm  = normalizar(pre)
                pre_clave = pre.upper().strip()
                if pre_norm in nombres_aprobados or pre_clave in claves_aprobadas:
                    disponibles.append({**base, "estado": f"Prerrequisito OK: {pre}"})
                else:
                    bloqueadas.append({**base, "prerrequisito": pre})
        ...
        cursor.execute("""
            ...
            LEFT JOIN (
                ...
                FROM materias WHERE alumno_id = ? AND estatus = 'APROBADA'
        ...
        for clave, info in rep_activas.items():
            if len(info["calendarios"]) >= MAX_REPROBACIONES:
                ...
                alertas.append({"tipo": "ARTICULO_33", "icono": "🔥",
                    "descripcion": (f"'{info['nombre']}' ({clave}) reprobada en "
        ...
        if creditos_requeridos > 0:
            if creditos_actuales >= umbral_practicas:
                alertas.append({"tipo": "PRACTICAS", "icono": "🔵",
                    "descripcion": (f"Con {creditos_actuales}/{creditos_requeridos} créditos "
                ...
                alertas.append({"tipo": "SERVICIO_PENDIENTE", "icono": "⏳",
                    "descripcion": (f"Servicio Social: te faltan "
                        f"{umbral_servicio - creditos_actuales} créditos "
                        f"({creditos_actuales}/{umbral_servicio}).")})

        if 0 < promedio_actual < 70:
            alertas.append({"tipo": "PROMEDIO_BAJO", "icono": "📉",
                "descripcion": f"Promedio {promedio_actual:.2f} por debajo de 70. Considera asesorías."})
        ...
        # ── 9. Imprimir reporte ───────────────────────────────────
        self._imprimir(alumno, pct_avance, disponibles, bloqueadas,
                       alertas, por_area, len(claves_aprobadas), rep_activas,
                       en_curso_claves, sin_equivalencia)

        return {
            "alumno": alumno, "disponibles": disponibles,
            "bloqueadas": bloqueadas, "alertas": alertas,
            "por_area": por_area, "sin_equivalencia": sin_equivalencia,
        }
    ...
    def _imprimir(self, alumno, pct_avance, disponibles, bloqueadas,
        ...
        if en_curso_claves:
            print(f" En curso (excluidas): {', '.join(sorted(en_curso_claves))}")
        print(sep2)
        ...
        if por_area:
            ...
            for a in por_area:
                ...
                print(f" {a['area']:<40} {a['total_plan']:>4} {a['aprobadas']:>4} "
